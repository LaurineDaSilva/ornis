...
export default {
  ...
  beforeMount() {
    this.initBirdsDashboard();
  },
  methods: {
    async initBirdsDashboard() {
...
<template>
  <section>
    <div class="mt-5 mb-4">
      <h1>{{ $t('birdsDashboard.titles.h1') }}</h1>
      <h2>{{ $t('birdsDashboard.titles.h2') }}</h2>
    </div>
      ...
      <ul v-if="loaded" class="row justify-content-between birds-list">
        <li v-for="bird in birds" :key="bird" class="col-md-6 shadow-sm card bird-cards">
          <img
            ...
            :alt="$t('birdsDashboard.imageAlt', { name: bird.commonName })"
          />
